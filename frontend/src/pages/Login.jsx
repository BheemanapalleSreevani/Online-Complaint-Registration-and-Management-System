    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center px-4 relative overflow-hidden text-slate-100">
      ...
      <div className="w-full max-w-md bg-zinc-900 rounded-3xl border border-zinc-800 shadow-2xl p-8 relative z-10 hover:shadow-indigo-500/5 transition-all duration-500">
        ...
        <div className="text-center">
          ...
          <h2 className="text-2xl font-extrabold text-slate-100 mt-4 tracking-tight">Welcome Back</h2>
          <p className="text-zinc-500 text-xs mt-1">Please authenticate to continue to your dashboard</p>
        </div>

        {/* Role Select Tabs */}
        <div className="mt-8 flex bg-zinc-950 p-1.5 rounded-xl border border-zinc-850">
          <button
            type="button"
            onClick={() => {
              setIsAdmin(false);
        ...
        {errorMsg && (
          <div className="mt-6 p-4 bg-rose-500/10 border border-rose-500/25 rounded-xl flex gap-3 text-rose-400 text-xs font-semibold animate-bounce leading-relaxed">
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-zinc-950 border border-zinc-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 hover:bg-zinc-850/30 transition-all font-medium text-slate-200 placeholder:text-zinc-600"
                required
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center">
              ...
              {!isAdmin && (
                <Link to="/forgot-password" className="text-[11px] font-bold text-brand-400 hover:text-brand-300 hover:underline transition-colors">
                  Forgot Password?
                </Link>
              )}
            </div>
        ...
        <div className="text-center mt-6 flex flex-col gap-2">
          {!isAdmin ? (
            <div>
              <span className="text-zinc-500 text-xs font-medium">New user? </span>
              <Link to="/register" className="text-brand-400 hover:text-brand-300 hover:underline text-xs font-bold transition-colors">
                Create Free Account
              </Link>
            </div>
          ) : (
            <div>
              <span className="text-zinc-500 text-xs font-medium">New officer? </span>
              <Link to="/register" className="text-brand-400 hover:text-brand-300 hover:underline text-xs font-bold transition-colors">
